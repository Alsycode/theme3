...
function Service3({serviceData}) {
	console.log("services",serviceData)

	const serv = serviceData?.data[0]?.attributes?.serviceitems;
      ...
      <>
		...
		<section className="content-inner">
			<div className="container">
				...
				<div className="row">
					{serviceData?.data[0]?.attributes?.serviceitems?.map((service) => (
						...
						<div className={`${open === "p1" ? "icon-bx-wraper style-5 box-hover text-center m-b30 active" : "icon-bx-wraper style-5 box-hover text-center m-b30"}`} onMouseOver={() => setOpen("p1")}>
							<div className="icon-bx"> 
								<span className="icon-cell"><i className={service.icon}></i></span> 
							</div>
							<div className="icon-content">
								<h4 className="dlab-title m-b15">{service.heading}</h4>
								<p className="m-b20">{service.description}</p>
